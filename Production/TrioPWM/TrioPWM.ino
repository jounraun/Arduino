...
int PWM_pin[PWM_CHANNELS + 1] = {0, 6, 9, 10};
...
void showhelp()
{
  Serial.println("Press +/-/u/d/1/2/3/m/a/h/r to control");
...
{ 
  ...
  if (PWM_current = 255 || PWM_current == 0) 
     PWM_current = 1;

  PWM_mode = EEPROM.read(1);
  if (PWM_mode == 255 || PWM_mode == 0) 
     PWM_mode = PWM_MANUAL;
}
